...
public class AndroidMusic implements Music, OnCompletionListener, OnSeekCompleteListener, OnPreparedListener, OnVideoSizeChangedListener {
    ...
    private int mediaPlayerCurrentPosition;//to resume in case of pause
    ...
    public void play() {
        if (this.mediaPlayer.isPlaying())
            return;
        //if we have some progress to the media player (this will mean that media player made pause) we need to resume when user try to play same music
        if(mediaPlayerCurrentPosition > 0){
            resume();
            return;
        }
        try {
